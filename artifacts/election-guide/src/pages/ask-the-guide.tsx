...
export default function AskTheGuide() {
  ...
  // If no conversation is active, show the conversation list/welcome
    ...
    return (
      <div className="container mx-auto px-4 py-8 max-w-4xl h-[calc(100vh-8rem)] flex flex-col">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Your AI assistant for election queries. Ask anything about the process.</p>
        </div>
        ...
        <div className="flex-1 flex flex-col md:flex-row gap-6 overflow-hidden">
          <Card className="flex-1 flex flex-col overflow-hidden border-2">
            <div className="p-4 border-b bg-muted/30 flex justify-between items-center">
              ...
              <Button onClick={handleCreateNew} size="sm" className="gap-2" disabled={createConversation.isPending}>
              ...
              {QUICK_REPLIES.map((reply) => (
                <Button
                  ...
                  onClick={() => {
                    ...
                    createConversation.mutate(
                      { data: { title: reply.substring(0, 30) + "..." } },
                      {
                        onSuccess: (newConv) => {
                          queryClient.invalidateQueries({ queryKey: getListAnthropicConversationsQueryKey() });
                          setActiveConversationId(newConv.id);
                          // We pass the initial message through a custom event or state
                          // Here we'll just set it to local storage to be picked up by the chat component
...
function ChatInterface({ conversationId, onBack }: { conversationId: number; onBack: () => void }) {
  ...
  // Handle initial message from quick replies
  useEffect(() => {
  ...
  const handleSend = async (text: string) => {
    ...
    queryClient.setQueryData(getGetAnthropicConversationQueryKey(conversationId), (oldData: any) => {
    ...
    try {
      ...
      // Force a refetch to get the actual persisted messages
            ...
            {conversation?.messages?.length === 0 && !isStreaming && (
              <div className="text-center py-10 px-4 max-w-md mx-auto">
                ...
                <p className="text-muted-foreground text-sm">
                  I can help answer questions about voter registration, polling booths, required documents, and the overall election process in India.
                </p>
              </div>
            )}
        ...
        <div className="p-4 bg-background border-t">
          ...
          >
            <Input
              ...
              placeholder="Ask about elections, voting, forms..."
              className="flex-1"
              disabled={isStreaming}
            />
            <Button type="submit" disabled={!inputValue.trim() || isStreaming} className="gap-2 shrink-0">
              <Send className="w-4 h-4" />
              <span className="hidden sm:inline">Send</span>
            </Button>
          </form>
          <p className="text-center text-xs text-muted-foreground mt-3">
            AI can make mistakes. For official information, always refer to the Election Commission of India website.
          </p>
        </div>
      </Card>
    </div>
